# ✈️ YatriLounge — Intelligent Airport Lounge Peak-Hour Predictor

> **What is YatriLounge?**
> It's an AI system that predicts how crowded an airport lounge will be in the next 6 hours. It then tells you exactly how many staff to deploy and how much food/drinks to prepare — reducing waste and improving guest experience.
...
**YatriLounge solves this** by using machine learning on flight schedules + historical lounge data to **forecast crowd levels 6 hours ahead** and automatically recommend staffing and food quantities.
...
| 📊 Interactive Chart | Beautiful line chart showing predicted crowd with peak hour highlighted |
...
| 🍽️ Menu Optimizer | Calculates exact snack (60%) and beverage (40%) units + 10% buffer |
...
python scripts/generate_sample_data.py   # Generate sample datasets
uvicorn app.main:app --reload --port 8000   # Start the API server
```
...
2. Upload `backend/data/flight_schedule.csv` → Flight Schedule box
3. Upload `backend/data/lounge_entries.csv` → Lounge Entries box
...
│   │   │   ├── predictions.py       #   → Train & forecast endpoints
│   │   │   ├── recommendations.py   #   → Staffing & menu endpoints
│   │   │   └── simulation.py        #   → What-if & PDF export
│   │   ├── services/                # Business logic
│   │   │   ├── feature_engineering.py  # → Extract ML features from data
│   │   │   ├── prediction_engine.py    # → Train model & make predictions
│   │   │   ├── staffing.py             # → Staff recommendation rules
│   │   │   ├── menu_optimizer.py       # → Food/beverage calculations
│   │   │   └── pdf_export.py           # → Generate PDF reports
│   │   └── utils/
│   │       └── validators.py        # CSV validation helpers
│   ├── scripts/
...
│   │   ├── components/
│   │   │   ├── Header.jsx           # Top bar with branding & actions
...
The project includes a **sample data generator** that creates realistic data so you can test immediately.
...
**Pattern**: ~15-25% of arriving passengers use the lounge. Entries are correlated with flight arrivals.
...
### Evaluation
The model reports **MAE** (Mean Absolute Error) and **RMSE** (Root Mean Square Error) — lower = better.
...
| `GET` | `/api/predict/forecast` | Get 6-hour crowd prediction |
| `GET` | `/api/predict/metrics` | Get MAE & RMSE scores |
...
## 🍽️ Menu Optimization Formula

```
For each predicted hour:
  Snack units   = predicted_crowd × 60% × 1.10 (buffer)
  Beverage units = predicted_crowd × 40% × 1.10 (buffer)
```
...
<p align="center">
  Built with ❤️ for smarter airport lounge management
</p>
